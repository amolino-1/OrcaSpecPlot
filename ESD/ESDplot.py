...
parser.add_argument(
    "-S",
    "--spectrumfile",  # Option keyword
    # Open and read the file specified by the argument
    type=argparse.FileType("r"),
    required=True,  # Indicate whether an argument is required or optional
    help="ESD output <basename>.spectrum",  # Help message
    metavar="\b",
)  # Alternate display name in help message
parser.add_argument(
    "-o",
    "--output",
    type=str,
    # Number of times argument can be used, '?' = single value optional
    nargs="?",
...
def energy_correction(value):
    # Planck constant (eV•s) * Speed of Light (m/s)
    electronvolt = 1239.8424122
...
fig, ax1 = plt.subplots(
    figsize=(cm_to_inch(12.5), cm_to_inch(6.99)), dpi=cm_to_inch(600)
)
...
ax1.plot(
    dataframe["Energy"],
    dataframe["TotalSpectrum"] / max(dataframe["TotalSpectrum"]),
    color="Black",
    linestyle="-",
    linewidth=1.0,
    marker="",
...
ax1.plot(
    energy_correction(dataframe["Energy"]),
    dataframe["TotalSpectrum"] / max(dataframe["TotalSpectrum"]),
    color="Red",
    linestyle="--",
    linewidth=1.0,
...
ax1.vlines(x=[x_max], ymin=[0], ymax=[1], colors=["Black"], linestyle="-", linewidth=1)
# Plot vertical line for λmax of energy corrected spectrum
ax1.vlines(
    x=[x_corrected_max_energy],
    ymin=[0],
    ymax=[1],
    colors=["Red"],
    linestyle="--",
